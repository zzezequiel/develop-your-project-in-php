...
INSERT INTO `userdata` (`idd`, `first_name`, `last_name`, `age`, `phone`, `city`, `avatar`) VALUES
(1, 'Admin', '', 0, 0, '', ''),
(2, 'Alex', 'Balaguer', 30, 677147018, 'Barcelona', 'assets\\img\\bala.jpg'),
(3, 'Ezequiel', 'Zvirgzdins', 19, 689982172, 'Barcelona', 'assets\\img\\perfil2.jpg'),
(4, 'Marta', 'Punset', 34, 696969696, 'Barcelona', 'assets\\img\\marta.jpg'),
(5, 'Alvaro', 'Alonso', 27, 661723881, 'Granada', 'assets\\img\\alvaro.jpg');
...
--
ALTER TABLE `products`
  ADD CONSTRAINT `products_ibfk_1` FOREIGN KEY (`id_user`) REFERENCES `userdata` (`idd`) ON UPDATE CASCADE;

--
-- Filtros para la tabla `users`
--
ALTER TABLE `users`
  ADD CONSTRAINT `users_ibfk_1` FOREIGN KEY (`user_id`) REFERENCES `userdata` (`idd`) ON UPDATE CASCADE;
COMMIT;
