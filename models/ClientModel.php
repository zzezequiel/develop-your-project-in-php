...
function getUserById($request){
    $query = $this->db->connect()->prepare("SELECT u.email, u.user_id, ud.first_name , ud.last_name, ud.age, ud.phone, ud.city, ud.avatar, p.title as 'title', p.location as 'location', p.description as 'description', p.pre_build, p.size, p.price, p.img as 'img', p.id
     FROM users u
